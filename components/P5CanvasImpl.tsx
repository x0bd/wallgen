  ...
  // Define sketch as a callback that can be referenced later
    ...
    class HexLattice {
      ...
      dict: Map<number, number> = new Map();
				stateCount: number = 3; // Will be updated based on palette length
      losesTo: Record<number, number> = {}; // Will be dynamically generated
        ...
        this.stateCount = stateCount;
					...
					// Create vectors for hex grid calculation (following wander.js)
        ...
        this.o.add(p.createVector(this.u.x, 0));
					...
					// Create the game rules - still rock-paper-scissors style but with any number of states
      ...
      createRules(stateCount: number): void {
        ...
        for (let i = 0; i < stateCount; i++) {
						this.losesTo[i] = (i + 1) % stateCount; // Each state loses to the next one
        }
      }
      ...
      *cells(): Generator<[number, number]> {
      ...
      // Update the lattice based on rock-paper-scissors rules
				// Adapted from wander.js's evolveLattice function to work with multiple states
      update(complexity: number): void {
        ...
        const nextDict = new Map();
					...
					for (let cell of this.cells()) {
						...
						// Initialize count to 0 for all states
          for (let i = 0; i < this.stateCount; i++) {
						...
						if (cellVal === undefined) cellVal = 0; // Default to first color if undefined
						...
						// This is similar to wander.js's: if (count[antagonist] >= 2)
      ...
      display(colors: any[]): void {
        ...
        const showBorder = false;
					...
					for (let cell of this.cells()) {
						...
						// Make sure the color index is within the range of available colors
      ...
      resetQuadtree();
        ...
        if (params.randomizeOnLoad) {
          ...
          time = p.random(0, 1000);
					...
					// Get all available foreground colors
					const availableColors = appColors.foregroundColors || [
						appColors.foreground,
					];
					// Use all colors in the palette for more variety
					const stateCount = availableColors.length;

					console.log(
						`Creating cellular automaton with ${stateCount} states`
					);

					// Create hex lattice with all states based on palette length
					...
					// Allow parameter-based pattern selection or random if not specified
					// Use complexity param to determine pattern type (1-5 range maps to 0-4)
					const patternType =
						Math.floor(normalizedParams.complexity / 2) % 5; // 0-4 different patterns

					// Create color sampling array with optimized distribution
					// This creates a more balanced but still varied distribution
					const colorDistribution = [];

					// Add primary colors more frequently, with slight emphasis on first colors
					for (let i = 0; i < stateCount; i++) {
						// Color weighting formula: more weight to first colors but all colors represented
						const weight = Math.max(
							1,
							Math.floor((stateCount - i) * 0.7 + 1)
						);
						for (let j = 0; j < weight; j++) {
							colorDistribution.push(i);
						}
					}

					// Pattern type 0: Gradient-like pattern (smoother transitions)
					if (patternType === 0) {
						...
						for (const cell of lattice.cells()) {
							const coords = lattice.cellCoords(cell);
							const distFromCenter = Math.sqrt(
								Math.pow(coords.x - centerX, 2) +
									Math.pow(coords.y - centerY, 2)
							);

							// Use distance to create gradient-like pattern
							const normalizedDist = distFromCenter / maxDist;
							const colorIndex = Math.floor(
								normalizedDist * stateCount
							);
							lattice.setValue(cell, colorIndex % stateCount);
						}
					}
					// Pattern type 1: Radial clusters (creates star-like formations)
					else if (patternType === 1) {
						// Create cluster centers
						const numClusters = Math.floor(p.random(3, 7)); // 3-6 clusters
						const clusters = [];
						...
						for (let i = 0; i < numClusters; i++) {
							clusters.push({
								x: p.random(p.width),
								y: p.random(p.height),
								color: Math.floor(p.random(stateCount)),
							});
						}
						...
						for (const cell of lattice.cells()) {
							...
							for (const cluster of clusters) {
								const dist = Math.sqrt(
									Math.pow(coords.x - cluster.x, 2) +
										Math.pow(coords.y - cluster.y, 2)
								);
							...
							// Assign color based on closest cluster with some randomness
							if (closestCluster && p.random() < 0.7) {
								lattice.setValue(cell, closestCluster.color);
							} else {
								// Some cells get random colors for variety
								const randomIndex = Math.floor(
									p.random(colorDistribution.length)
								);
								lattice.setValue(
									cell,
									colorDistribution[randomIndex]
								);
							}
						}
					}
					// Pattern type 2: Striped pattern (creates linear designs)
					else if (patternType === 2) {
						const stripeWidth = Math.floor(p.random(3, 8)); // Width of stripes
						const angle = p.random(p.TWO_PI); // Random orientation

						for (const cell of lattice.cells()) {
							const coords = lattice.cellCoords(cell);

							// Calculate position along the stripe direction
							const pos =
								coords.x * Math.cos(angle) +
								coords.y * Math.sin(angle);

							// Determine stripe index and color
							const stripeIndex = Math.floor(
								pos / (stripeWidth * cellSize)
							);
							const colorIndex = Math.abs(
								stripeIndex % stateCount
							);

							lattice.setValue(cell, colorIndex);
						}
					}
					// Pattern type 3: Perlin noise-based pattern (organic, cloud-like)
					else if (patternType === 3) {
						// Use density parameter to influence noise scale (more compact vs. spread out)
						const noiseScale =
							0.005 + (normalizedParams.density / 100) * 0.015;

						for (const cell of lattice.cells()) {
							const coords = lattice.cellCoords(cell);

							// Use Perlin noise to create organic patterns
							const noiseVal = p.noise(
								coords.x * noiseScale,
								coords.y * noiseScale
							);
					...
					// Pattern type 4: Balanced random with clusters (better distribution than pure random)
					else {
						// Use density parameter to influence cluster chance (higher density = more clusters)
						const clusterChance =
							0.1 + (normalizedParams.density / 100) * 0.3; // 0.1-0.4

						for (const cell of lattice.cells()) {
							// Sample from our weighted distribution
							const randomIndex = Math.floor(
								p.random(colorDistribution.length)
							);
							const value = colorDistribution[randomIndex];
							lattice.setValue(cell, value);

							// Small chance to create cell clusters for more organic patterns
							if (p.random() < clusterChance) {
								// Propagate this color to some neighbors for cluster formation
								let count = 0;
								for (const neighbor of lattice.neighbors(
									cell
								)) {
									if (p.random() < 0.7 && count < 2) {
										lattice.setValue(neighbor, value);
										count++;
									}
								}
							}
        ...
        particles = [lattice];
					...
					console.log(
						`Initialized artistic cellular automaton with pattern: ${patternNames[patternType]}`
					);
    ...
    p.draw = () => {
      ...
      const currentIsSaving = isSavingRef.current;
        ...
        p.background(bgR, bgG, bgB);
					...
					// Uses all foreground colors from the palette
        let cellColors: any[] = [];
          ...
          cellColors = [
					...
					// Set frame rate exactly like wander.js
					const cellFrameRate = (normalizedParams.speed - 1) * 6 + 1;
					p.frameRate(cellFrameRate);

					// Update and display - exactly like wander.js behavior
					// Always update on each frame like in wander.js
