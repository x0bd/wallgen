...
interface AlgorithmContextState {
	...
	updateCustomColor: (
		id: string,
		color: Omit<ColorOption, "id" | "isCustom">
	) => void;
...
const AlgorithmContext = createContext<AlgorithmContextState | undefined>(
	undefined
);
...
export function AlgorithmProvider({ children }: { children: ReactNode }) {
	...
	const [colorOptions, setColorOptions] =
		useState<ColorOption[]>(defaultColorOptions);
	...
	const updateCustomColor = useCallback(
		(id: string, color: Omit<ColorOption, "id" | "isCustom">) => {
	...
	const getCurrentColors = useCallback(() => {
		...
		return {
			...
			foreground: isInverted
				? selectedColor.background
				: selectedColor.foreground,
			foregroundColors:
				selectedColor.foregroundColors && isInverted
	...
	const exportCanvas = useCallback(
		(options: {
			...
			window.dispatchEvent(
				new CustomEvent("wallgen-capture-canvas", {
			...
			setTimeout(() => {
				window.dispatchEvent(
					new CustomEvent("wallgen-export-canvas", {
						detail: {
							...options,
							algorithm,
							params,
...
export function useAlgorithm() {
	...
	if (context === undefined) {
		throw new Error(
			"useAlgorithm must be used within an AlgorithmProvider"
		);
	}
	return context;
}
